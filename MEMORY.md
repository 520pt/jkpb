...
- 2026-07-22: Template shift recognition must keep white or near-white single `中` cells as empty (`-` in the UI). Only colored shift cells, especially green cells, should become `中`/`早`/`晚`; automatic recheck should reclassify each current grid cell by its stored image box.
