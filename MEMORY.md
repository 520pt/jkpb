...
- 2026-08-16: 人员中心状态卡片不能把状态和详情硬塞进一个小标签；已改为更宽自适应卡片，状态标签只显示已绑定/已配置/已启用等短状态，手机号、UserID、班前提醒、搭档姓名等详情放在旁边单独展示并用省略号兜底。
...
- 2026-09-04: 排班 OCR 修复覆盖 2026 年 8 月/9 月真实截图：顶部标题 OCR 参与月份识别；绿色格优先按字形区分 `中/早/晚`，避免阈值漂移把罗越等人的 `中` 识别成 `早`；白/灰底 `出差` 同时支持横排和竖排版式。自动核对新增 `baseline_grid`，凡当前编辑值与上传识别基线不同即视为人工修改，不再覆盖或报错。Dockerfile/compose 增加 apt/pip 加速参数，默认 apt 使用阿里云 Debian、pip 使用阿里云 PyPI；清华 PyPI 曾对 setuptools 返回 403，不作为默认源。
- 2026-09-05: 自动核对保留人工修改，但不能只“保留不管”。如果人工修改后的值仍与重新识别结果冲突，后端会返回 `manual_conflict`，前端用黄框提示，避免把手改错值静默当成正确答案。
